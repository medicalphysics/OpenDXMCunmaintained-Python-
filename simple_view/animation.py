...
def mix2(image, dose, ax1, ax2, norm=900000., cut=30000.):
    k1, k2 = -500., 1000.
    ...
    ax2.imshow(im_a)#, vmin=-100, vmax=300)
...
def interpolate_z(arr, i, left=True):
    sh = arr.shape
    if not (0 < i < sh[2]):
...
def interpolate_y(arr, i, scale=1., left=True):
    sh = arr.shape
    if not (0 < i < sh[1]):
...
def make_animation(array, dose, name='mov'):
...
#            fig.clear()
            ax1.cla()
            ax2.cla()
            arr_slice = interpolate_z(array, i)
            dose_slice = interpolate_z(dose, i)
#            arr_slice = np.squeeze(array[:,:,i])
#            dose_slice = np.squeeze(dose[:,:,i])
            mix2(arr_slice, dose_slice, ax1, ax2, dmax, dmin)
...
#        for i in range(array.shape[1]):
        for i in np.linspace(0,array.shape[1]-2,array.shape[1]*2):
            ax1.cla()
            ax2.cla()
            arr_slice = interpolate_y(array, i)
            dose_slice = interpolate_y(dose, i)
#            arr_slice = np.squeeze(array[:,i,:])
#            dose_slice = np.squeeze(dose[:,i,:])
            mix2(arr_slice, dose_slice, ax1, ax2, dmax, dmin)
...
def make():
    name = 'Golem'
    dens, dose = read_arrays("C://test//data.h5")
